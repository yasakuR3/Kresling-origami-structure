...
def main():
    ...
    origamis = np.zeros((3, k, 2, n, 3), dtype=float)

    # ======= ここから下は、あなたのアルゴリズムそのまま =======
    origamis[0, 0, 0, 0, 0] = (a / 2.0) / np.tan(np.radians(180/n))
    origamis[0, 0, 0, 0, 1] = a / 2.0
    origamis[0, 0, 0, 0, 2] = 0

    origamis[0, 0, 1, 0, 0] = origamis[0, 0, 0, 0, 0] - b * np.sin(np.radians(180 - θ)) * np.cos(np.radians(180 - r))
    origamis[0, 0, 1, 0, 1] = origamis[0, 0, 0, 0, 1] + b * np.cos(np.radians(180 - θ)) 
    origamis[0, 0, 1, 0, 2] = b * np.sin(np.radians(180 - θ)) * np.sin(np.radians(180 - r))
    ...
    for i in range(0, n):
        # 底面A
        triangles.append(((1, 0, 0, i), (2, 0, 0, i), (2, 0, 0, (i+1)%n)))
        triangles.append(((1, 0, 0, (i+1)%n), (1, 0, 0, i), (2, 0, 0, (i+1)%n)))

        # 上面B
        triangles.append(((1, k-1, 1, i), (2, k-1, 1, i), (2, k-1, 1, (i+1)%n)))
        triangles.append(((1, k-1, 1, (i+1)%n), (1, k-1, 1, i), (2, k-1, 1, (i+1)%n)))

        triangles.append(((1, 0, 0, (i+1)%n), (1, 0, 0, i), (1, 0, 1, (i+1)%n)))
        triangles.append(((2, 0, 0, (i+1)%n), (2, 0, 0, i), (2, 0, 1, (i+1)%n)))

        triangles.append((((1, 0, 0, i), (1, 0, 1, i), (1, 0, 1, (i+1)%n))))
        triangles.append((((2, 0, 0, i), (2, 0, 1, i), (2, 0, 1, (i+1)%n))))
    
    for i in range(1, k):
        for j in range(0, n):
            if i % 2 == 1:
                triangles.append(((1, i, 1, j), (1, i-1, 1, j), (1, i-1, 1, (j+1)%n)))
                triangles.append(((1, i, 1, j), (1, i, 1, (j+1)%n), (1, i-1, 1, (j+1)%n)))

                triangles.append(((2, i, 1, j), (2, i-1, 1, j), (2, i-1, 1, (j+1)%n)))
                triangles.append(((2, i, 1, j), (2, i, 1, (j+1)%n), (2, i-1, 1, (j+1)%n)))

            else:
                triangles.append(((1, i-1, 1, j), (1, i-1, 1, (j+1)%n), (1, i, 1, (j+1)%n)))
                triangles.append(((1, i-1, 1, j), (1, i, 1, j), (1, i, 1, (j+1)%n)))

                triangles.append(((2, i-1, 1, j), (2, i-1, 1, (j+1)%n), (2, i, 1, (j+1)%n)))
                triangles.append(((2, i-1, 1, j), (2, i, 1, j), (2, i, 1, (j+1)%n)))
    ...
    for g in range(1, 3):
        # for i range(0, k):
        for j in range(0, n):
            edges.append(((g, 0, 0, j), (g, 0, 0, (j+1)%n)))
            edges.append(((g, 0, 1, j), (g, 0, 1, (j+1)%n)))
            edges.append(((g, 0, 0, j), (g, 0, 1, j)))
            edges.append(((g, 0, 0, (j+1)%n), (g, 0, 1, j)))
